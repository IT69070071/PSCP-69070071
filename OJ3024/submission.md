...
```text
โจทย์ให้หา คะแนนที่น้อยที่สุดแล้วตรวจสอบว่าคะแนนที่น้อยที่สุดห่างกับคะแนนที่มากที่สุด เกิน2มั้ย

input:คะแนนรวม คะแนนที่มากที่สุด

output:ถ้าคะแนนที่น้อยที่สุดห่างกับคะแนนที่มากที่สุด เกิน2 ให้print "Surprising"ถ้าไม่ให้print"Not surprising"

constraints: โจทย์ให้คะแนน เป็นจำนวนจริง (float)
```
...
```text
step1:รับค่าคะแนนรวม
step2:รับค่าคะแนนที่มากที่สุด
step3:หาว่าคะแนนที่น้อยที่สุดคือเท่าไหร่ โดยเอาคะแนนรวม-ค่ามากสุด หาร2
step4:นำคะแนนที่มากที่สุดลบคะแนนที่น้อยที่สุด 
step5:ถ้าค่าที่ได้มากกว่า2 print(Surprising) ถ้าน้อยกว่าprint(Not surprising)
```
...
```text
ใช้เหมือนแผนแรก แต่เปลี่ยนstep3 หาค่าที่น้อยที่สุด จากเดิมคือเอาคะแนนรวม-คะแนนที่มากที่สุด หาร2 เป็น เอาคะแนนรวม-(คะแนนที่มากที่สุด*2) แทน เพราะสมการเดิมจะไม่ได้คะแนนที่น้อยที่สุด และใส่function max(0,...) ไว้กรณีที่ค่าที่ได้ติดลบ จะต้องเป็น0
```
...
```text
ทดสอบกรณีที่ผลต่างมากกว่า 2 ต้องแสดง Surprising
```
