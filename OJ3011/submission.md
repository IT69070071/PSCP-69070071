...
```text
โจทย์ให้รับค่าข้อความ2ข้อความ คือ "Red" "Yellow" หรือ "Blue" แล้วให้เปรียบเทียบกรณีว่า ถ้าข้อความแรกเป็น... และข้อความที่2เป็น... ให้print ค่าออกมาตามที่กำหนด ถ้าค่าที่รับมาไม่ใช่ "Red" "Yellow" หรือ "Blue" ให้print "Error"

input: ข้อความ "Red" "Yellow"หรือ"Blue" 2บรรทัด

output:"Orange" ถ้่าinputเป็น"Red"และ"Yellow" , "Violet" ถ้่าinputเป็น"Red"และ"Blue" ,"Green" ถ้่าinputเป็น"Yellow"และ"Blue" ถ้าค่าที่รับมาไม่ใช่ "Red" "Yellow" หรือ "Blue" ให้print "Error"

constraints:
ถ้า input เป็นค่าเดียวกันทั้งสอง output ควรจะออกมาเป็นค่าเดิม
```
...
```text
1.รับค่า  "Red" "Yellow" หรือ "Blue" 
2.ตรวจสอบว่าค่าที่ได้ตรงตามเงื่อนไขไหน 
3.ถ้าค่าที่รับมีค่าเหมือนกัน ให้print ค่านั้น
4.print ค่าที่ตรงตามเงื่อนไข
5.ถ้าค่าที่รับไม่ใช่ "Red" "Yellow" หรือ "Blue" ให้print "Error"
```
...
```text
ใช้เหมือนกับแผนแรก คือ รับค่ามาตรวจเงื่อนไขว่าตรงตามเงื่อนไขไหนแล้วให้print ค่าที่ได้ตามเงื่อนไขนั้น ถ้าค่าที่รับเหมือนกันให้printค่าเดิม ถ้าค่าที่รับไม่ใช่ "Red" "Yellow" หรือ "Blue" ให้print "Error" ตอนแรกใช้Or แต่ติดเงื่อนไขที่ว่าถ้าค่าที่รับอันนึงคือ "Red" "Yellow" หรือ "Blue" แต่อีกค่าไม่ใช่ ระบบจะไม่printอะไรออกมา จึงเปลี่ยนมาใช้And เพื่อให้ครอบคลุมทุกกรณี
```
...
```text
เพราะค่าทั้งสองเป็นค่าเดียวกัน
```
...
```text
เพราะค่านึงอยู่ในเงื่อนไข"Red" "Yellow" หรือ "Blue" แต่อีกค่าไม่ได้อยู่
```
...
```text
เพราะเป็นค่าที่ไม่ใช่ "Red" "Yellow" หรือ "Blue" 
```
