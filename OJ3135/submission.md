...
```text
โจทย์ให้วนตัวเลข1-N โดยเริ่มจาก1 แล้ว+K ทุกครั้งที่+K จะนับcount+1 ถ้าตัววนถึงเลข1 จะหยุดแล้วprint count แต่ถ้าวนถึงT count จะ+1ก่อน แล้วค่อยprint

input:
N(แทนจำนวนตัวเลขในวง)K(แทนจำนวนที่บวกวน)T(แทนจำนวนตัวเลขที่เลือก) ในบรรทัดเดียว

output:
count ที่บวกในแต่ละรอบวนเลข

constraint:
ถ้าถึงเลข1 จะprint countเลย แต่ถ้าถึงเลขTที่เลือก จะบวกcount ก่อน เวลา+Kถ้าเกินจำนวนN จะวนกลับมาที่1
```
...
```text
step1:รับค่า N K T ในบรรทัดเดียว(โดยNแทนจำนวนตัวเลขทั้งหมด,Kแทนจำนวนเลขที่+ ,Tแทนจำนวนเลขที่เลือก)
step2:กำหนดcountเป็น0
step3:กำหนดเลขปัจจุบันคือ 1
step4:บวกค่าcount
step5:วนลูปค่าจนกว่าจะวนมาถึง1 หรือเลขที่เลือกT
step6:ถ้าเป็นเลขที่เลือกT จะ+count ก่อนprint ถ้าเป็นเลข1 จะprint count เลย
```
